...
콘서트 티켓팅 트래픽 폭주 상황에서 **유량 제어**를 위한 대기열 시스템입니다.

### 왜 Redis인가? (vs DB vs MQ)

| 기준 | DB (MySQL) | MQ (Kafka) | **Redis (선택)** |
|------|------------|------------|------------------|
| **순위 조회** | ❌ `COUNT(*)` O(N) | ❌ 오프셋 기반, 실시간 불가 | ✅ `ZRANK` O(log N) |
| **실시간 순위 변동** | ❌ 매번 재계산 필요 | ❌ 순서 보장만, 순위 개념 없음 | ✅ ZSET 자동 정렬 |
| **토큰 상태 관리** | ✅ CRUD 용이 | ❌ 메시지 수정 불가 | ✅ HASH로 메타데이터 관리 |
| **활성화 처리** | ✅ UPDATE | ❌ Consumer 구조와 맞지 않음 | ✅ ZREM + SADD 원자적 |
| **확장성** | 중간 | 높음 | 높음 |

**선택 이유:**
- 대기열의 핵심 기능은 **"내 앞에 몇 명?"** → `ZRANK` O(log N)이 결정적
- Kafka는 Consumer 모델이라 **실시간 순위 조회 불가** (오프셋 ≠ 순위)
- MySQL은 100만 건 기준 `COUNT(*)` 쿼리가 **수백 ms** 소요
...
```
┌─────────────────────────────────────────────────────────────┐
│ queue:waiting:{concertId}  [ZSET]                          │
│   용도: 대기열 순서 관리 (score = 등록 timestamp)            │
│   예시: {"abc-123": 1706444400000, "def-456": 1706444401000}│
├─────────────────────────────────────────────────────────────┤
│ queue:active:{concertId}  [SET]                            │
│   용도: 활성화된 토큰 집합 (입장 허용된 사용자)              │
│   예시: {"ghi-789", "jkl-012"}                              │
├─────────────────────────────────────────────────────────────┤
│ queue:token:{token}  [HASH]                                │
│   용도: 토큰 메타데이터 (userId, status, expiresAt 등)      │
└─────────────────────────────────────────────────────────────┘
```
...
사용자 요청 → 활성 슬롯 확인 (SCARD) 
    ...
    슬롯 여유 있음           슬롯 없음
         │                     │
         ▼                     ▼
   ACTIVE 상태로 저장      WAITING 상태로 저장
   (SADD active SET)       (ZADD waiting ZSET)
         │                     │
         ▼                     ▼
   rank=0, 즉시 입장      ZRANK로 순위 반환
```

**스케줄러 (1초마다):** WAITING → ACTIVE 전환
```
KEYS queue:waiting:* → 각 콘서트별 빈 슬롯 확인 → ZPOPMIN으로 상위 N개 활성화
```
...
| 연산 | MySQL | Redis |
|------|-------|-------|
| 순위 조회 | O(N) `COUNT(*)` | **O(log N)** `ZRANK` |
| 상위 N개 조회 | O(N log N) | **O(log N + M)** `ZRANGE` |
| 토큰 저장 | O(log N) INSERT | **O(log N)** `ZADD` |

**100만 건 기준:** MySQL 순위 조회 ~150ms → Redis ~0.1ms (**1500배 개선**)

---

### 영속성 설정

Redis 재시작 시 데이터 유실 방지를 위해 **AOF** 영속성 설정:

```conf
# docker/redis/redis.conf
appendonly yes
appendfsync everysec  # 최악의 경우 1초 데이터만 유실
```
