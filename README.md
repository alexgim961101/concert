...
|-----------|---------|---------------|-----------|
| **좌석 예약** | `ReserveSeatUseCase` | 동시에 같은 좌석 예약 → 중복 예약 | DB 비관적 락 (`SELECT FOR UPDATE`) |
...
❌ Redis 분산 락: DB 레코드 락이 더 간단하고 트랜잭션과 자연스럽게 통합
✅ 비관적 락: 한 명만 성공해야 하는 명확한 시나리오, 충돌 즉시 차단
```

**선택 근거:**
- 콘서트 티켓팅은 특성상 **동일 자원(좌석)에 대한 경쟁이 매우 높음**
- 낙관적 락 사용 시 재시도 폭주 → 오히려 성능 저하
- `SELECT FOR UPDATE`로 **먼저 락을 획득한 사람만 예약 성공**, 나머지는 대기 후 실패
- DB 트랜잭션과 자연스럽게 통합되어 구현이 간단
...
#### 왜 Redis 분산 락을 선택하지 않았나?

| 기준 | Redis 분산 락 | DB 락 (현재 선택) |
|------|---------------|-------------------|
| 추가 인프라 | Redis 필수 | 기존 DB 활용 |
| 트랜잭션 통합 | 별도 관리 필요 | 자연스럽게 통합 |
| 복잡도 | TTL, 갱신 로직 필요 | 간단 |
| 적합한 상황 | 외부 API Rate Limiting, 캐시 동기화 | DB 레코드 락 |

현재 요구사항(DB 레코드에 대한 동시성 제어)에서는 **DB 락이 더 간단하고 안전**합니다.
Redis 분산 락은 향후 다음과 같은 요구사항 발생 시 도입 검토:
- 외부 PG사 API 호출 Rate Limiting
- 분산 캐시 동기화
- 스케줄러 중복 실행 방지
